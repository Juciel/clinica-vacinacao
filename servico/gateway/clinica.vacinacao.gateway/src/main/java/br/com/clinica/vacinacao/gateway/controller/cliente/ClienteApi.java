...
@Api(value = "Gateway API de Clinicas de Vacinacao", produces = MediaType.APPLICATION_JSON_VALUE)
public interface ClienteApi {
    ...
    @GetMapping(value = "/gateway/v1/clinica/vacinacao/clientes", produces = MediaType.APPLICATION_JSON_VALUE)
    ResponseEntity<List<Cliente>> getClientes();

    @ApiOperation(value = "Endpoint para consultar um cliente.")
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = "Processo realizado com sucesso", response = Cliente.class),
            @ApiResponse(code = 401, message = "Autenticação não realizada / token inválida"),
            @ApiResponse(code = 403, message = "Usuario não possui permissão para acessar esta API"),
    ...
    @GetMapping(value = "/gateway/v1/clinica/vacinacao/cliente", produces = MediaType.APPLICATION_JSON_VALUE)
